@props(['name' => null, 'wireModel' => null, 'placeholder' => '••••••••', 'required' => true, 'class' => '', 'id' => null])

<div class="password-input-wrapper w-full">
    <input type="password" @if ($wireModel) wire:model.live.blur="{{ $wireModel }}" @endif @if ($name) name="{{ $name }}" @endif @if ($id) id="{{ $id }}" @endif placeholder="{{ $placeholder }}" class="input w-full pr-10 hide-password-reveal {{ $class }}" {{ $attributes->except(['class', 'id', 'name', 'wire-model', 'placeholder', 'required']) }} @if ($required) required @endif>
    <button type="button" class="password-toggle-btn text-base-content/50 hover:text-base-content transition-colors" tabindex="-1" aria-label="Toggle password visibility">
