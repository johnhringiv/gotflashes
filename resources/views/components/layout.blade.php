...
<nav class="navbar shadow-md" style="background-color: var(--color-primary); color: var(--color-primary-content);">
            ...
            <ul tabindex="0" class="menu dropdown-content mt-3 z-50 p-3 shadow-lg bg-base-100 rounded-box w-56 text-base-content">
                ...
                @auth
                    @if(auth()->user()->is_admin)
                        <li><a href="/admin/awards" class="text-base py-3 {{ str_starts_with(request()->path(), 'admin/awards') ? 'active font-bold text-accent' : '' }}">Awards Admin</a></li>
                    @endif
                @endauth
    ...
    <div class="navbar-center hidden lg:flex">
        <ul class="menu menu-horizontal px-1">
            ...
            @auth
                @if(auth()->user()->is_admin)
                    <li><a href="/admin/awards" class="btn btn-ghost btn-sm hover:bg-white/10 {{ str_starts_with(request()->path(), 'admin/awards') ? '!text-white !font-bold underline decoration-accent decoration-2 underline-offset-4' : 'text-white/80' }}">Awards Admin</a></li>
                @endif
            @endauth
        </ul>
    </div>
