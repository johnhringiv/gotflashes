            ...
            @can('update', $flash)
                @if ($flash->isEditable($minDate, $maxDate))
                    <div class="flex gap-1">
                        <button type="button" wire:click="openEditModal({{ $flash->id }})" class="btn btn-ghost btn-xs">
                            Edit
                        </button>
                        <button type="button" wire:click="delete({{ $flash->id }})" wire:confirm="Are you sure you want to delete this flash?" class="btn btn-ghost btn-xs text-error">
                            Delete
                        </button>
                    </div>
                @endif
